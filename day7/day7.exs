defmodule Day7 do
  ...
  def eval_equation([a, operator, b | rest]) do
    case operator do
      ...
      :* -> eval_equation([a * b | rest])
      :|| -> eval_equation([String.to_integer("#{a}#{b}") | rest])
    end
  end

  def verify_equation_part1(test_value, equation) do
    build_equations(equation, [:+, :*], [])
    |> Enum.map(&eval_equation/1)
    |> Enum.any?(fn result -> result == test_value end)
  end

  def verify_equation_part2(test_value, equation) do
    build_equations(equation, [:+, :*, :||], [])
    |> Enum.map(&eval_equation/1)
    |> Enum.any?(fn result -> result == test_value end)
  end
  ...
  def part1 do
    ...
    equations
      |> Enum.filter(fn {test_value, equation} -> verify_equation_part1(test_value, equation) end)
  ...
  def part2 do
    ...
    equations
      |> Enum.filter(fn {test_value, equation} -> verify_equation_part2(test_value, equation) end)
      |> Enum.reduce(0, fn {test_value, _}, acc -> acc + test_value end)
      |> IO.inspect 
  end
end
